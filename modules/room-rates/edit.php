...
if (!$rateId) {
    redirect(APP_URL . '/modules/room-rates/index.php');
}
...
$stmt = $db->prepare("
    ...
    WHERE rr.id = ? AND rr.deleted_at IS NULL
");
$stmt->execute([$rateId]);
$roomRate = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!checkCsrfToken($_POST['_csrf_token'] ?? '')) {
        ...
        if (!$roomId) {
            ...
            // Check if room already has this rate plan (excluding current record)
            $stmt = $db->prepare("SELECT id FROM room_rates WHERE room_id = ? AND rate_plan_id = ? AND id != ? AND deleted_at IS NULL");
            $stmt->execute([$roomId, $ratePlanId, $rateId]);
            if ($stmt->fetch()) {
        ...
        if (!$error) {
            try {
                $stmt = $db->prepare("
                    UPDATE room_rates 
                    SET room_id = ?, rate_plan_id = ?, base_price = ?, weekend_price = ?, 
                        extra_adult_price = ?, extra_child_price = ?, tax_included = ?, 
                ...
                logActivity('update', 'room_rates', "Updated room rate ID: {$rateId}");
                $success = 'Room rate updated successfully.';
                
                // Refresh room rate data
                $stmt = $db->prepare("
                    SELECT rr.*, 
                           r.room_number, r.room_name,
                           rp.name as rate_plan_name, rp.code as rate_plan_code
                ...
                if (DEBUG_MODE) {
                    error_log("Edit room rate error: " . $e->getMessage());
                }
        ...
        <main class="content-area">
            <div class="container-fluid">
                <div class="page-header">
                    <h1 class="page-title">Edit Room Rate</h1>
                    <p class="page-subtitle">Edit room rate for <?php echo htmlspecialchars($roomRate['room_number']); ?> - <?php echo htmlspecialchars($roomRate['rate_plan_name']); ?></p>
                </div>
                        ...
                        <form action="" method="POST">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="room_id" name="room_id" required>
                                        ...
                                        <?php foreach ($rooms as $room): ?>
                                            <option value="<?php echo $room['id']; ?>" <?php echo $roomRate['room_id'] === $room['id'] ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="rate_plan_id" name="rate_plan_id" required>
                                        ...
                                        <?php foreach ($ratePlans as $plan): ?>
                                            <option value="<?php echo $plan['id']; ?>" <?php echo $roomRate['rate_plan_id'] === $plan['id'] ? 'selected' : ''; ?>>
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="base_price" name="base_price" required step="0.01" min="0" value="<?php echo $roomRate['base_price']; ?>">
                                    <small class="form-text text-muted">Standard nightly rate</small>
                                </div>
                                
                                <div class="col-md-3 mb-3">
                                    <label for="weekend_price" class="form-label">Weekend Price</label>
                                    <input type="number" class="form-control" id="weekend_price" name="weekend_price" step="0.01" min="0" value="<?php echo $roomRate['weekend_price'] ?: ''; ?>">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="extra_adult_price" name="extra_adult_price" step="0.01" min="0" value="<?php echo $roomRate['extra_adult_price'] ?: ''; ?>">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="extra_child_price" name="extra_child_price" step="0.01" min="0" value="<?php echo $roomRate['extra_child_price'] ?: ''; ?>">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="tax_included" name="tax_included" <?php echo $roomRate['tax_included'] ? 'checked' : ''; ?>>
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" <?php echo $roomRate['is_active'] ? 'checked' : ''; ?>>
                            ...
                            <div class="d-flex gap-2 mt-4">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-save me-2"></i>Update Room Rate
                                </button>
